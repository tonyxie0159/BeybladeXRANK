...
同隊零件去重證據：快速對戰會拒絕同一玩家三顆配置中重複的 `PartId`，且失敗時不保存半套 Lineup；Tournament 團體賽會拒絕跨隊友重複零件，改用獨立零件後三種團體規則的排序、物化與重排測試均通過。PostgreSQL 使用交易範圍 advisory lock 序列化同場提交。

2026-09-05 隔離 SQLite 測試站完成兩場快速對戰：同一顆「CX 試驗機」分別選擇 v1 與 v2，Round 全程保留選定版本；兩場 4:0 完賽後母陀螺合計 2 勝、得分 4，版本明細正確拆為 v1 得分 3 與 v2 得分 1。含重複 `1-50`、`J` 的陣容在網頁顯示錯誤並拒絕提交；小量選項下不再顯示六個多餘搜尋欄。
